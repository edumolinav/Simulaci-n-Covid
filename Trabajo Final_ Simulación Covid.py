...
"""
    Se declaran las constantes:
        ...
        b : tiene un rango de [1.5,2.5] de acuerdo a OMS ó puede ser [2,3]
        dias : numero de dias 
        N = numero de datos para un intervalo de dias
        delta pasos entre numero de dias
"""
...
axis.set_position((0.1,0.33,0.8,0.6))

### Se configuran los Slider
k_ax = plt.axes((0.1,0.23,0.8,0.025),facecolor="aliceblue")
k_sl = Slider(ax=k_ax,label="k",valmin=k_range[0],valmax=k_range[1],valinit= k,valstep=1/14,orientation="horizontal",color="steelblue")
b_ax = plt.axes((0.1,0.19,0.8,0.025),facecolor="aliceblue")
b_sl = Slider(ax=b_ax,label="b",valmin=b_range[0],valmax=b_range[1],valinit= b,valstep=0.02,orientation="horizontal",color="steelblue")
...
def animacion(y,t):          # Genera la animación.
    ...
    axis.set_xlabel('tiempo (dias)', fontsize=14)
    axis.set_ylabel('Proporción de la población',fontsize=14)
